...
function show_response(event) {
	...
	$.get(
		...
		function (result) {
			...
			$this.hide().before('<span class="button">收起</span>');
			$parent.after($result);
			$('span.button',$parent).click(function () {
				$result.remove();
				$(this).remove();
				$this.show();
			});
			$('.button',$result).click(function (event) {
				add_response(event,href);
			});
			$('.rspinput',$result).focus(clear);
		}
	);
	event.preventDefault();
	event.stopPropagation();
...
function add_response(event,href) {
	...
	else {$this.prev().val('内容不能为空！').css({color:'#FF0000'});}
}
$(function () {
